...
- **2026-09-21「和美的痕跡」，共 26 張投影片**。番外篇（當天主活動是「走跳卡里善」尋寶遊戲，投影片是補充教材）：許常惠家族在和美的線索、和美街長宿舍的命名爭議（1896 年原為警察官吏派出所，「街長宿舍」疑似戰後追認的稱呼）、附近街區的都市計畫年代對照，最後留給學生一份資料檢索任務清單。
- 2026-09-14「該去買車票了嗎？」，共 24 張投影片。從第一週「要不要留彰化」接下去：彰化二戰以來人口淨移出、清代劉銘傳在台中規劃省城、1895 年民政支部四個月內從彰化遷往台中、路徑依賴，最後回到「和美需要什麼」。
...
`app.js`／`styles.css` 的快速跳頁功能（數字＋Enter 跳頁、`G` 或點頁碼開目錄）已於 2026-09-21 隨 `research-writing-class` 同步進來；目錄由 `app.js` 讀投影片標題自動生成，新增單元不必手寫目錄。若之後 `research-writing-class` 的 `app.js` 再更新，記得先 `diff` 再整份覆蓋，並回頭檢查舊單元的 `#counter`（須為 `<button>`）與操作說明 `<dl>` 是否也補上新按鍵說明。

## 設計與內容說明

- 正文逐字保留 Obsidian 原稿的語氣；補充、老師自己的例子與史料批判放在講者備註，投影片上不出現。
- 2026-09-21 單元沒有既有的逐字原稿，是根據老師當天口頭需求直接查證撰寫。內容分兩段：許常惠家族在和美的線索（家族背景查得到，汎愛醫院確切位置與許常惠本人是否談過和美童年查無資料）；和美街長宿舍的命名爭議（1896 年原為警察官吏派出所，「街長宿舍」一名疑似戰後追認，但另有訪談說法認為庄長本來就住派出所裡，兩說並陳未下定論）。查無資料與未交叉考證的部分一律標在講者備註，投影片標題保留問號。第 23 張留了四題資料檢索任務給學生，都是備課時真的查不到答案的問題。
